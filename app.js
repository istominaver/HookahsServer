...
app.get('/restaurantsList', function(req, res) {
	...
	ddb.scan(params, function(err, data) {
    	if (err) res.status(500).json({"err":err});
      		...
      		res.status(200).json({"action":"restaurantsList","result":"ok","reqestId":new Date().getTime(),"requestData":req,"data": {"restaurantsList":tempArray}});//add result ok/err + errCode + errDescr
    	}
	});
});
    ...
    if (err) res.status(500).json({"err":err,"restaurantId":restaurantId});
    	...
    	ddb.scan(paramsMixes, function(err, data) {
   			if (err) res.status(500).json({"err":err,"restaurantId":restaurantId}); // an error occurred
   				...
   				res.status(200).json({"action":"mixesList","result":"ok","reqestId":new Date().getTime(),"requestData":req,"data":{"categoriesList":categoriesList}, "mixesList": tempArray});
   			}
   		});
    }
 });
...
//form and return orderId - unixtime
        ...
        ddb.putItem({
            ...
            else res.status(200).json({"action":"makeOrder","result":"ok","reqestId":new Date().getTime(),"requestData":req,"data": {"orderId": orderId}});                
            }
        );
 });
